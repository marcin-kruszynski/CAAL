"""
LlamaCppLLM Plugin for LiveKit Agents
=====================================

llama.cpp server integration with OpenAI-compatible API.
...
class LlamaCppLLM(llm.LLM):
    """
    LiveKit LLM plugin for llama.cpp server with OpenAI-compatible API.

    This plugin is designed to be used with a VoiceAssistant that overrides
    the llm_node method. The actual LLM calls are handled by llamacpp_llm_node(),
    which supports MCP tool discovery and execution.

    The LlamaCppLLM class:
    ...
    Args:
        model: Model name (e.g., "gpt-oss-20b-mxfp4")
        ...
        base_url: llama.cpp server URL (OpenAI-compatible endpoint)

    Example:
        >>> llm = LlamaCppLLM(model="gpt-oss-20b-mxfp4", temperature=0.7)
    ...
    def __init__(
        ...
        *,
        ...
        top_k: int = 20,
        num_ctx: int = 8192,
        base_url: str = "http://llama.home/v1",
    ) -> None:
        super().__init__()
        self._model = model
        ...
        logger.debug(f"LlamaCppLLM initialized: {model} (num_ctx={num_ctx})")
    ...
    def chat(
        ...
        *,
        ...
        is bypassed. The llm_node override calls llamacpp_llm_node() directly.

        This implementation exists for interface compliance and fallback.
        """
        return _LlamaCppLLMStream(
            llm=self,
            chat_ctx=chat_ctx,
            tools=tools or [],
    ...
    async def aclose(self) -> None:
        """Cleanup (no-op for llama.cpp)."""
        pass


class _LlamaCppLLMStream(llm.LLMStream):
    """
    ...
    async def _run(self) -> None:
        ...
        llm_node override handles all LLM interactions via llamacpp_llm_node().
        ...
        logger.warning(
            "LlamaCppLLM._run() called directly - this usually means llm_node "
